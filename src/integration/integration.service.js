const Integration = require('../core/db/schemas/integration.model');
const ResyncJob = require('../core/db/schemas/resync-job.model');
...
/**
 * Manual re-sync trigger by user id — resolves the user here so the HTTP layer
 * never touches the database.
 * @throws {Error} 404-tagged when the user does not exist
 */
const startResyncForUser = async (userId, serviceName) => {
  ...
  return startResync(user, serviceName.toLowerCase());
};
